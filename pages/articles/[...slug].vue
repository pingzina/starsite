...
const { data } = await useFetch<ArticlePayload>(`/api/articles/${slug.value}`)
...
useSeoMeta({
  ...
  ogImage: data.value.article.cover || '/og-cover.svg',
})
</script>

<template>
  <div class="grid gap-6 lg:grid-cols-[260px_minmax(0,1fr)_220px]">
    <SidebarCategories :active="data?.article.category" />

    <article v-if="data?.article" class="panel p-6 sm:p-8">
      ...
      <header class="border-b border-slate-100 pb-6">
        <div class="flex flex-wrap items-center gap-2">
          <span class="rounded-full bg-teal-50 px-3 py-1 text-xs font-medium text-teal-700">{{ data.article.category }}</span>
          <span v-for="tag in data.article.tags" :key="tag" class="rounded-full bg-slate-100 px-3 py-1 text-xs text-slate-600">
            #{{ tag }}
          </span>
        </div>
        <h1 class="mt-4 text-4xl font-semibold text-slate-950">{{ data.article.title }}</h1>
        <p class="mt-4 text-base leading-8 text-slate-600">{{ data.article.description }}</p>
        <div class="mt-5 flex flex-wrap gap-5 text-sm text-slate-500">
          <span>{{ t('label.updated') }} {{ data.article.updatedAt }}</span>
